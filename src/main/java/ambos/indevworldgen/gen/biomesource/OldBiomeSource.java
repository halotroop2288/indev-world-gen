...
public class OldBiomeSource extends BiomeSource {
	...
	private HeightRetriever parent = HeightRetriever.NONE;

	public OldBiomeSource(long seed, OverworldChunkGeneratorConfig config) {
		BiomeLayerSampler[] samplers = OldBiomeLayers.build(seed, config);

		this.oceanSampler = samplers[0];
		this.landSampler = samplers[1];
	}
	
	public void setHeightRetriever(HeightRetriever retriever) {
		this.parent = retriever;
	}
